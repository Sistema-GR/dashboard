<template>
  <div class="mb-6 w-full">
    <button @click="open = !open" class="w-full text-left px-6 py-3 justify-between bg-blue-50 rounded-[10px] focus:outline-none flex items-center">
      <span class="text-25 font-bold text-black">Impacto do Processo de Recursos</span>
      ...
      <div class="flex flex-col lg:flex-row gap-10 px-10 py-8 justify-center items-stretch">
        ...
        <div class="flex flex-col gap-10 flex-1 min-w-[300px] max-w-[320px] justify-center">
          <div class="bg-white rounded-[10px] shadow-md flex flex-col w-full max-w-full">
            ...
            <div class="flex items-center justify-center py-8 text-3xl font-bold"
                 :class="pessoasAfetadas >= 0 ? 'text-green-600' : 'text-red-600'">
              {{ pessoasAfetadas >= 0 ? '+' : '' }}{{ pessoasAfetadas }}
            </div>
          </div>
          <div class="bg-white rounded-[10px] shadow-md flex flex-col w-full max-w-full">
            <div class="bg-[#3459A2] text-white text-center font-bold text-15 p-3 rounded-t-[10px]">
            ...
            <div class="flex items-center justify-center py-8 text-25 font-bold">
              R$ {{ valorCorrigido }}
            </div>
          </div>
        </div>
...
export default {
  ...
  setup(props) {
    ...
    const generateBlueColorPalette = (count) => {
      const darkest = '#1e3a8a'  
      const lightest = '#dbeafe'
    ...
    const createOrUpdateChart = async () => {
      if (!chartDistribuicao.value || !open.value) {
        return;
      }

      if (!props.distribuicao || props.distribuicao.length === 0) {
        destroyChart();
        return;
      }
      ...
      chartInstance = new Chart(chartDistribuicao.value, {
        ...
        options: {
          ...
          scales: {
            ...
            y: { beginAtZero: true, grid: { color: '#e5e7eb' }, ticks: { font: { size: 12 }, stepSize: 10 } }
          }
        }
      });
    };

    watch(() => props.distribuicao, () => {
      createOrUpdateChart();
    }, { deep: true });
