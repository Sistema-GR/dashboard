<template>
  <div class="mb-6 w-full">
    <button @click="open = !open" class="w-full text-left px-6 py-3 justify-between bg-blue-50 rounded-[10px] focus:outline-none flex items-center">
      <span class="text-25 font-bold text-black">Dados Completos</span>
      ...
      <div class="px-10 py-8">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div class="p-6 overflow-x-auto">
            <table class="min-w-full text-15 text-left border-separate border-spacing-y-1">
              <thead>
                <tr class="bg-[#2360a5] text-white">
                  <th class="p-2 font-bold">Endereço de e-mail</th>
                  <th class="p-2 font-bold">Nome completo</th>
                  <th class="p-2 font-bold">Unidade de atuação</th>
                  <th class="p-2 font-bold">Categoria</th>
                  <th class="p-2 font-bold">Equipe Responsável</th>
                  <th class="p-2 font-bold">Responsável</th>
                </tr>
              </thead>
              <tbody class="bg-white text-black">
                <tr v-for="(item, index) in tableData" :key="index" class="hover:bg-gray-50">
                  <td class="p-2">{{ item.email }}</td>
                  <td class="p-2">{{ item.nomeCompleto }}</td>
                  <td class="p-2">{{ item.unidadeAtuacao }}</td>
                  <td class="p-2">{{ item.categoria }}</td>
                  <td class="p-2">{{ item.equipeResponsavel }}</td>
            ...
            <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-200">
              ...
              <div class="flex gap-2">
                <button 
                  ...
                  class="px-3 py-1 border border-gray-300 rounded text-15 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  ←
                </button>
                <button 
